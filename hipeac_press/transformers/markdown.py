...
"""
    ...
    def _ordered_list_to_markdown(element):
        items = "\n".join(f"1. {process_text(item)}" for item in element.items)
        return f"{items}\n"
    ...
    @staticmethod
    def _reference_to_markdown(element):
        return element.text
        ...
        if self.document.references:
            ...
            for ref in self.document.references:
                if ref.text.startswith("["):
                    # Extract the key (number or text) from between brackets
                    key = ref.text[1 : ref.text.index("]")]
                    ref_text = ref.text[ref.text.index("]") + 1 :].strip()

                    # Sort into numbered or text references
                    try:
                        num = int(key)
                        numbered_refs.append((num, ref_text))
                    except ValueError:
                        text_refs.append((key, ref_text))

            # Sort and write numbered references first
            for num, text in sorted(numbered_refs):
                md += f"[^{num}]: {text}\n"

            # Sort and write text references
            for key, text in sorted(text_refs):
                md += f"[^{key}]: {text}\n"
